                        ...
                        <div className="flex-1">
                            ...
                            >
                                ...
                                <div className="flex md:hidden flex-col justify-between h-full">
                                    {/* Top Section - Icon and Plan Info in a tight row */}
                                    <div className="flex flex-row items-center space-x-3 mb-2.5">
                                        {/* Animated icon container - slightly smaller */}
                                        <motion.div
                                            animate={{ rotate: [0, 5, -5, 0], scale: [1, 1.05, 1] }}
                                            transition={{ duration: 2, repeat: Infinity, ease: "easeInOut" }}
                                            className="relative p-2.5 bg-[#c47c1e] rounded-md border border-[#a0651a] shadow-sm shrink-0"
                                        >
                                        ...
                                        <div className="text-left flex-1 min-w-0">
                                            <h2 className="text-base sm:text-lg font-bold bg-gradient-to-r from-amber-700 to-yellow-600 bg-clip-text text-transparent truncate leading-tight">
                                                {planData?.name || "Premium Plan"}
                                            </h2>
                                            <p className="text-[11px] sm:text-xs text-amber-600/80 font-medium flex items-center space-x-1 mt-0.5">
                                                <TrendingUpIcon className="w-3 h-3 shrink-0" />
                                                <span className="truncate">Updated {planData?.updated_at ? new Date(planData.updated_at).toLocaleDateString() : 'Never'}</span>
                                            </p>
                                        </div>
                                    </div>

                                    {/* Mobile Action Buttons - Side-by-side row */}
                                    <div className="flex flex-row gap-2 pt-2 border-t border-yellow-400/20">
                                        {/* Download Button */}
                                        <motion.button
                                            onClick={handleDownload}
                                            whileHover={{ scale: 1.02 }}
                                            whileTap={{ scale: 0.95 }}
                                            className="flex-1 inline-flex items-center justify-center space-x-1.5 px-2 py-1.5 bg-gradient-to-r from-amber-600 to-yellow-500 text-white text-[11px] sm:text-xs font-medium rounded hover:from-amber-700 hover:to-yellow-600 transition-all shadow-sm"
                                        >
                                        ...
                                        <motion.button
                                            ...
                                            whileHover={{ scale: 1.02 }}
                                            whileTap={{ scale: 0.95 }}
                                            className="flex-1 inline-flex items-center justify-center space-x-1.5 px-2 py-1.5 bg-gradient-to-r from-red-600 to-red-500 text-white text-[11px] sm:text-xs font-medium rounded hover:from-red-700 hover:to-red-600 transition-all shadow-sm"
                                        >
